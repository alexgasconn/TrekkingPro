...
const App: React.FC = () => {
  ...
  // Hourly slice covering the hike, from start time through the estimated finish time
  const tripHourlyForecast = useMemo(() => {
    ...
    return weather.hourlyForecast.filter((entry: HourlyForecast) => {
      const t = new Date(entry.time);
      return t.getTime() >= startFloor.getTime() && t.getTime() <= finish.getTime();
    });
